...
must_haves:
  truths:
    ...
    - "API timeouts are configurable via config"
    - "Config validation runs on app startup"
  ...
  artifacts:
    ...
    - path: "frontend/src/config/validation.js"
  ...
  key_links:
    ...
    - from: "frontend/src/main.js"
      to: "frontend/src/config/validation.js"
      via: "import and call"
      pattern: "validateConfig"
---

<objective>
Create centralized frontend constants module for HNF1B domain values and enhance config with API timeouts and validation.

Purpose: Extract hardcoded magic numbers from frontend utilities into documented constants for maintainability, reusability, and consistency with backend. Add API timeout configuration and startup validation.

Output: `frontend/src/constants/` module with domain-specific constant files, enhanced `frontend/src/config/` with timeouts and validation
</objective>
...
<task type="auto">
  <name>Task 2: Add API timeouts and config validation</name>
  <files>
frontend/src/config/app.js
frontend/src/config/validation.js
frontend/src/main.js
  ...
  <action>
Enhance frontend config with API timeouts (QUAL-17) and add startup validation (QUAL-18).

**Update `frontend/src/config/app.js`:**
Add TIMEOUTS section to API_CONFIG:
...
**Create `frontend/src/config/validation.js`:**
```javascript
/**
...
import { API_CONFIG, VIZ_CONFIG, FEATURES } from './app.js';
...
Add config validation call after imports, before app mount:
...
This ensures config issues are caught early in development without breaking production.
  </action>
  <verify>
Run: `cd /home/bernt-popp/development/hnf1b-db/frontend && npm run lint && npm run dev` - starts without errors, validation runs in dev mode
  </verify>
  <done>
API timeouts in config, validation runs on startup in dev mode
  </done>
</task>

<task type="auto">
  <name>Task 3: Update frontend files to use constants</name>
  <files>
frontend/src/utils/dnaDistanceCalculator.js
...
2. API timeouts defined in config with env var override support
3. Config validation runs on dev startup
4. `npm run lint` passes in frontend directory
5. `npm test` passes in frontend directory
6. No hardcoded magic numbers remain in modified files
7. dnaDistanceCalculator.js still exports same values (backward compatible)
</verification>

<success_criteria>
- [x] `frontend/src/constants/` module exists with documented constants
- [x] `frontend/src/config/app.js` has API timeouts (QUAL-17)
- [x] `frontend/src/config/validation.js` validates config on startup (QUAL-18)
