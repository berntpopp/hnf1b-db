...
async def _aggregate_with_total(
    ...
    """Helper function to perform an aggregation that returns both grouped 
...
async def _aggregate_latest_report_field(
    ...
    latest_report_subq = select(
        Report.individual_id,
        Report.id.label("report_id"),
        getattr(Report, report_field).label("field_value"),
        func.row_number()
        .over(partition_by=Report.individual_id, order_by=desc(Report.report_date))
        .label("rn"),
    ).subquery()

    # Get individuals with their latest report field value
    latest_reports = select(latest_report_subq.c.field_value).where(
    ...
    if report_field == "age_onset":
        latest_reports = (
            select(
                func.case(
                    (
                        func.lower(latest_report_subq.c.field_value) == "prenatal",
                        "prenatal",
                    ),
                    (
                        func.lower(latest_report_subq.c.field_value) == "not reported",
                        "not reported",
                    ),
                    else_="postnatal",
                ).label("field_value")
            )
            .select_from(latest_report_subq)
...
async def count_variants_by_newest_verdict(
    ...
    latest_classification_subq = select(
    ...
    latest_verdicts = select(latest_classification_subq.c.verdict).where(
        latest_classification_subq.c.rn == 1
    )
...
async def count_phenotypes_by_described(
    ...
    results = [
        {"phenotype_id": row.phenotype_id, "name": row.name, "counts": row.counts}
...
async def count_variants_by_impact_group(
    ...
    result = await db.execute(query)
    grouped_counts = [{"_id": row._id, "count": row.count} for row in result.fetchall()]
    ...
    query = text("""
        ...
        individual_phenotypes AS (
            SELECT
                individual_id,
                CASE
                    WHEN phenotypes ? 'HP:0000002' AND 
                         phenotypes->'HP:0000002'->>'described' = 'yes' THEN 'Abnormal'
                    ELSE 'Normal'
