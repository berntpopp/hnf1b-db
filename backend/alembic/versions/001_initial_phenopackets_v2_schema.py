...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('phenopacket', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        ...
        sa.Column('schema_version', sa.String(20), nullable=False, server_default='2.0.0'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('phenopacket_id')
    )

    # Create indexes for phenopackets
    op.create_index(op.f('ix_phenopackets_phenopacket_id'), 'phenopackets', ['phenopacket_id'], unique=True)
    op.create_index(op.f('ix_phenopackets_subject_id'), 'phenopackets', ['subject_id'])
    op.create_index(op.f('ix_phenopackets_subject_sex'), 'phenopackets', ['subject_sex'])

    # Create GIN indexes for JSONB queries
    op.create_index('idx_phenopacket_jsonb', 'phenopackets', ['phenopacket'], postgresql_using='gin')
    op.create_index(
    ...
    op.create_index(
        ...
        'phenopackets',
        [sa.text("jsonb_path_query_array(phenopacket, '$.phenotypic_features[*].type.id')")],
        postgresql_using='gin'
    )
    ...
    op.create_index(
        ...
        'phenopackets',
        [sa.text("jsonb_path_query_array(phenopacket, '$.interpretations[*].diagnosis.genomic_interpretations[*].variant_interpretation.variation_descriptor.label')")],
        postgresql_using='gin'
    )
    ...
    op.create_index(op.f('ix_phenopacket_audit_phenopacket_id'), 'phenopacket_audit', ['phenopacket_id'])
