...
async def fixture_sample_phenopackets_for_search(fixture_db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_search_enhanced_full_text_query_returns_matches(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    ...
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?q=ZZTEST_UNIQUE_RENAL", headers=fixture_auth_headers
    )
...
@pytest.mark.asyncio
async def test_search_enhanced_hpo_filter_returns_matches(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    """Test search with HPO ID filter."""
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?hpo_id=HP:0000118", headers=fixture_auth_headers
    )
    assert response.status_code == 200
    data = response.json()["data"]
...
@pytest.mark.asyncio
async def test_search_enhanced_sex_filter_returns_matching_sex(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    ...
    """
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?sex=MALE&limit=100", headers=fixture_auth_headers
    )
    assert response.status_code == 200
    data = response.json()["data"]
...
@pytest.mark.asyncio
async def test_search_enhanced_gene_filter_returns_matching_gene(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    ...
    """
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?gene=HNF1B&limit=100", headers=fixture_auth_headers
    )
    assert response.status_code == 200
    data = response.json()["data"]
...
@pytest.mark.asyncio
async def test_search_enhanced_pmid_filter_returns_matches(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    """Test search with PMID filter."""
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?pmid=123", headers=fixture_auth_headers
    )
    assert response.status_code == 200
    data = response.json()["data"]
...
@pytest.mark.asyncio
async def test_search_enhanced_combined_filters_returns_matches(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
...
@pytest.mark.asyncio
async def test_search_enhanced_no_results_returns_empty(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    """Test search with a query that yields no results."""
    response = await fixture_async_client.get(
        "/api/v2/phenopackets/search?q=nonexistentphenotype", headers=fixture_auth_headers
    )
    assert response.status_code == 200
    data = response.json()["data"]
    assert len(data) == 0
...
@pytest.mark.asyncio
async def test_search_enhanced_pagination_returns_correct_pages(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
    ...
    response_page2 = await fixture_async_client.get(
...
@pytest.mark.asyncio
async def test_search_enhanced_rank_by_relevance_false_returns_results(
    fixture_async_client: AsyncClient, fixture_sample_phenopackets_for_search, fixture_auth_headers
):
