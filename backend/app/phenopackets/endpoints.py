...
async def get_phenopackets_batch(
    ...
    Returns:
        List of phenopacket documents (excludes soft-deleted records)
...
async def get_by_publication(
    ...
    **Error Codes:**
    ...
    # Build query with JSONB filtering (exclude soft-deleted records)
    query = """
        SELECT
            phenopacket_id,
            phenopacket
...
async def search_phenopackets(
    ...
    """Advanced search across phenopackets (excludes soft-deleted records)."""
    # Exclude soft-deleted records
    base_query = select(Phenopacket).where(Phenopacket.deleted_at.is_(None))
...
async def delete_phenopacket(
    ...
    """Soft delete a phenopacket with audit trail.
...
async def aggregate_by_feature(
    ...
    total = sum(row._mapping["count"] for row in rows)

    return [
        AggregationResult(
            label=row.label or row.hpo_id,
            count=row._mapping["count"],
            percentage=(row._mapping["count"] / total * 100) if total > 0 else 0,
