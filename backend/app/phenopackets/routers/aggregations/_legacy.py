...
- Uses PostgreSQL materialized views when enabled (config.yaml)
- Falls back to live JSONB queries if views unavailable
- Views: mv_feature_aggregation, mv_disease_aggregation, mv_sex_distribution
"""
...
async def _check_materialized_view_exists(
...
async def aggregate_by_feature(
    ...
    if await _check_materialized_view_exists(db, "mv_feature_aggregation"):
...
async def aggregate_by_disease(
    ...
    """
    # Try materialized view first (O(1) indexed lookup)
    if await _check_materialized_view_exists(db, "mv_disease_aggregation"):
...
async def aggregate_sex_distribution(
    ...
    """
    # Try materialized view first (O(1) indexed lookup)
    if await _check_materialized_view_exists(db, "mv_sex_distribution"):
