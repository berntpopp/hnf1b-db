...
class DirectSheetsToPhenopackets:
    ...
    def _create_publication_reference(self, publication_id: str) -> Optional[Dict[str, Any]]:
        ...
        if pub_data is None:
            # If no publication data found, return None
            # This ensures we don't create references with PUB:pubXXX format
            return None
    ...
    async def load_google_sheets(self) -> None:
        ...
        try:
            ...
            # Create a mapping of Publication ID to full publication data
                ...
                if value and value.lower() not in ["no", "not reported", "unknown", ""]:
                    ...
                    if row.get("Publication"):
                        ...
                        if pub_ref:
                            ...
                            # This should not happen if publication map is loaded correctly
                            # But if it does, we'll skip adding a reference rather than using raw pub ID
                            pass
    ...
    def build_phenopacket(
        ...
        # Create ExternalReference for each unique publication
        for pub_id in sorted(pub_ids):
            pub_ref = self._create_publication_reference(pub_id)
            if pub_ref:
