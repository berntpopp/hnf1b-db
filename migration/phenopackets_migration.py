...
class PhenopacketsMigrationFixed:
    ...
    def _load_phenotype_mappings(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            ...
            "BrainAbnormality": {
                "id": "HP:0002060",
                "label": "Abnormality of the brain",
            },
            ...
            "KidneyBiopsy": {
                "id": "HP:0100820",
                "label": "Kidney biopsy showing abnormalities",
            },
    ...
    def _parse_varsome_notation(self, varsome_str: str) -> Dict[str, Optional[str]]:
        ...
        if match:
            return {
                ...
                "p_dot": f"p.{match.group(4)}"
                if match.group(4)
                else None,  # p.Val61Gly
            }
                    ...
                    if pd.notna(individual_id):
                        ...
                        variant_info = {
                            **parsed,
                            ...
                            "detection_method": row.get("DetecionMethod")
                            or row.get("DetectionMethod"),
    ...
    async def transform_to_phenopackets(
        ...
        for individual in tqdm(
            data["individuals"], desc="Transforming to phenopackets"
        ):
            # Get related data
            ind_id = individual["id"]
            individual_id = individual["individual_id"]  # e.g., "ind0001"

            reports = [r for r in data["reports"] if r["individual_id"] == ind_id]
            ...
            db_variants = [v for v in data["variants"] if v["individual_id"] == ind_id]
    ...
    def _build_subject(
        ...
        if reports:
            latest_report = max(
                reports, key=lambda r: r.get("report_date") or datetime.min
            )
    ...
    def _build_phenotypic_features(
        ...
        for report in reports:
            ...
            for phenotype_key, phenotype_data in phenotypes.items():
                ...
                # Create feature
                feature = {"type": {"id": mapping["id"], "label": mapping["label"]}}
                ...
                if described == "no":
                    ...
                    if (
                        phenotype_key == "HP:0012622"
                        and "stage" in str(described).lower()
                    ):
                        try:
                            import re
                ...
                if age_onset and age_onset != "not reported":
                    feature["onset"] = {"age": self._parse_age_to_iso8601(age_onset)}
    ...
    def _build_diseases(self, reports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        diseases.append({"term": self.disease_mappings["hnf1b"]})
        ...
        for report in reports:
            ...
            if "MODY" in phenotypes:
                ...
                if mody.get("described") not in ["no", "not reported", None]:
                    ...
                    # Avoid duplicates
                    if not any(
                        d["term"]["id"] == disease_entry["term"]["id"] for d in diseases
                    ):
    ...
    def _build_interpretations(
        ...
        for idx, variant in enumerate(variants):
            ...
            if variant.get("transcript") and variant.get("c_dot"):
                ...
                expressions.append({"syntax": "hgvs.c", "value": variant["c_dot"]})

            if variant.get("p_dot"):
                expressions.append({"syntax": "hgvs.p", "value": variant["p_dot"]})

            if variant.get("hg38"):
                expressions.append({"syntax": "hgvs.g", "value": variant["hg38"]})
                    ...
                    "genomicInterpretations": [
                        {
                            ...
                            "variantInterpretation": {
                                ...
                                "variationDescriptor": {
                                    ...
                                    if variant.get("segregation") == "De novo"
                                    else None,
                                },
                            },
                        }
        ...
        for report in reports:
            ...
            if doi and doi not in seen_dois:
                ...
                file_entry = {
                    ...
                    "description": report.get("title", "Publication reference")[:200]
                    if report.get("title")
                    else "Publication reference",
                    "fileAttributes": {
    ...
    def _build_metadata(
        ...
        if comments:
            ...
            metadata["comments"] = comments[
                :5
            ]  # Limit to first 5 comments to avoid bloat
