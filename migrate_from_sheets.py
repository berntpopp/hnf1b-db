...
def import_individuals():
    ...
    # Optionally, rename columns for age_reported and cohort if they exist.
    ...
    if 'report_id' not in df.columns:
        ...
        for idx, row in report_rows.iterrows():
            try:
                ...
                # Assume that the phenotype information is stored in a column named "Phenotypes"
    ...
    if 'VariantType' not in df.columns:
        ...
        for idx, row in variant_rows.iterrows():
            try:
                ...
                variant_data['is_current'] = True  # Default: mark as current
                variant_data['variant_type'] = row.get('VariantType')
                variant_data['variant_reported'] = row.get('VariantReported')
                variant_data['ID'] = row.get('ID')
                variant_data['hg19_INFO'] = row.get('hg19_INFO')
                variant_data['hg19'] = row.get('hg19')
                variant_data['hg38_INFO'] = row.get('hg38_INFO')
                variant_data['hg38'] = row.get('hg38')
                variant_data['varsome'] = row.get('Varsome')
                variant_data['detection_method'] = row.get('DetecionMethod')
                variant_data['segregation'] = row.get('Segregation')
